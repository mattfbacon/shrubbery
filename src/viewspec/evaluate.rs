...
impl ViewSpec {
	...
	fn make_query(&self, after: Option<models::FileId>, limit: i64) -> (String, Bindings<'_>) {
		...
		let query = format!("SELECT DISTINCT files.id, files.name FROM file_tags LEFT JOIN files ON files.id = file_tags.file WHERE {} AND file_tags.file > {} ORDER BY file_tags.file LIMIT {}", query, after.unwrap_or(-1), limit);
		(query, bindings)
	}
