//! This module centers around `Location` and `Span` which together can be used to track locations and spans within byte streams.
...
/// Pwease do not make `end` less than `start` or Fewwis will be angwy.
///
/// We do not use the `std::ops` type because it is known to have several serious API design issues.
...
impl Span {
	...
	/// Create an arbitrary span to use when there is no actual span, for example when parsing from tokens directly
	#[must_use]
